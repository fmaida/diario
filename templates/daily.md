# {{day_name_short_2}} {{day}} {{month_name}} {{year}}

### Cosa ho nella testa oggi?

1. 

### Cosa ho intenzione di fare oggi?

1. 

### Che cosa ho imparato oggi?

1. 

### Perché questo dovrebbe importarmi?

1. 

### Che cosa farò di diverso a causa di questo?

1. 


#### Vai alla vista settimanale:
[{{monday_day_name_short_2}} {{monday_day}} {{monday_month_name}} – {{sunday_day_name_short_2}} {{sunday_day}} {{sunday_month_name}} {{sunday_year}}]({{weekly_url}})

#### Vai al giorno precedente:
[{{yesterday_day_name_short_2}} {{yesterday_day}} {{yesterday_month_name}}]({{yesterday_url}})

#### Vai al giorno successivo:
[{{tomorrow_day_name_short_2}} {{tomorrow_day}} {{tomorrow_month_name}}]({{tomorrow_url}})